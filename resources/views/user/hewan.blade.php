...
<section class="container mx-auto mt-12 text-center px-6">
    ...
    <div class="flex justify-center gap-4 mt-4">
        {{-- Tombol filter. Atur active-filter di JS setelah dimuat --}}
        ...
        @forelse($hewansTersedia as $h)
            <div class="hewan-item bg-white p-6 border border-[#8b5e34] rounded-lg shadow-md flex flex-col justify-between h-full transition-transform duration-300 hover:scale-105" data-jenis="{{ $h->jenis }}">
                <div class="w-full h-56 flex items-center justify-center bg-white">
                    <img loading="lazy" src="{{asset('storage/gambar_hewan/' . $h->gambar) }}" alt="Foto {{ $h->nama }}" class="w-full h-full object-contain rounded-md">
                </div>
                ...
                <div class="flex justify-center space-x-4 mt-4">
                    @auth
                        ...
                        <a href="{{ route('adopsi.form', ['hewan_id' => $h->id]) }}" {{-- Contoh: route('adopsi.form') --}}
...
{{-- Galeri Adopsi Berhasil (Terhubung dengan Database) --}}
    ...
    <div class="flex flex-wrap justify-center gap-6">
        @forelse($hewansDiadopsi as $hewan)
            <div class="shadow-lg rounded-xl overflow-hidden w-40 h-40 flex items-center justify-center bg-white">
                {{-- Pastikan path gambar benar, sesuaikan jika berbeda --}}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Atur filter "Semua" sebagai default saat halaman dimuat
    applyFilter('Semua');
